...
def export(model, reason_model, reason_labels, vocab, snap, raw, metrics,
    ...
    bundle = dict(
        ...
        approval=dict(trees=trees, base=base, **meta,
                      # عتبة تصنيف «موافقة غير كاملة»، مختارة على شريحة تحقّق
        ...
        # نسبة الاعتماد الفعلية لكل نتيجة (المبلغ المغطى ÷ إجمالي مبلغ الطلب)،
        # تُحسب من البيانات التاريخية وتُستخدم لتقدير المبلغ المتوقّع اعتماده للطلب.
...
def _recovery_ratios(df):
    """
    نسبة الاعتماد الوسيطة (المبلغ المغطى ÷ إجمالي مبلغ الطلب) لكل فئة هدف.
    فئة «موافقة غير كاملة» تجمع المرفوضة والمقبولة جزئياً، فتُحسب نسبتها
...
def _write_model_card(m, art_dir, bundle):
    ...
    lines += [
        ...
        "إيراد فعلية — نسبة الاعتماد الوسيطة فيها نحو النصف مقابل ~93% للمقبولة",
        ...
        "وهو التقييم الأمين لنموذج سيُستخدم على طلبات مستقبلية.",
        "",
    ...
    lines += [
        ...
        f"- استرجاع «موافقة غير كاملة»: **{dep.get('recall_nfa')}** "
        f"(دقّتها {dep.get('precision_nfa')})",
        f"- استرجاع «موافقة كاملة»: **{dep.get('recall_approved')}**",
        ...
        "",
        "استرجاع فئة «موافقة غير كاملة» هو المقياس التشغيلي الأهم: نسبة طلبات الموافقة",
    ...
    lines += [
        ...
        "التدريب بنافذة **متوسّعة زمنياً** — أي من طلبات الموافقة السابقة فقط — لمنع تسرّب",
