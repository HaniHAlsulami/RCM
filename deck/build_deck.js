...
/* ============ 1 · TITLE ============ */
{
  ...
  s.addNotes("افتتاحية: سديد يجيب عن سؤال واحد قبل رفع الطلب لشركة التأمين، وهو هل سيُعتمد بالكامل. المنصّة مبنيّة بالكامل وجاهزة، والعرض اليوم عن العائد لا عن التمويل.");
}
...
/* ============ 2 · EXECUTIVE SUMMARY ============ */
{
  ...
  s.addText([
    ...
    { text:"المطلوب قرار تشغيلي واحد، وهو إدخال فحص سديد خطوةً إلزامية قبل رفع الطلب لشركة التأمين.", options:{} },
...
/* ============ 4 · WHY TODAY'S PROCESS FAILS ============ */
{
  ...
  flow(2.02, "اليوم", RED, [
    ...
    ["الخطوة 2","تردّ شركة التأمين بالرفض أو الخصم"],
  ...
  s.addText("التصحيح قبل الرفع أرخص من إعادة الطلب بعد الرفض، فهو لا يستهلك دورة مراجعة لدى شركة التأمين ولا يؤخّر بدء الخدمة.",
...
/* ============ 7 · PREDICTION → ACTION ============ */
{
  ...
  s.addText("مقابل 17.7 بالمئة لو خُمِّن السبب اعتباطاً. والنموذج لا يقرأ المرفقات، بل يرتّب الأرجح من أنماط 19,741 طلباً سابقاً.",
...
/* ============ 11 · VALUE PATH ============ */
{
  ...
  const steps = [
    { n:"1,000",   l:"طلب يُرفَع لشركة التأمين",            w:11.9, c:"DCE6EF", tc:NAVY, sub:"" },
...
/* ============ 13 · COST & LIMITS ============ */
{
  ...
  col(X2, SURF, "الحدود", AMBER, [
    ["مساعد قرار لا بديل عن المراجعة","لا يُتّخذ قرار برفض خدمة لمريض بناءً عليه إطلاقاً، وهو لا يميّز الرفض الكامل من الخصم الجزئي."],
    ...
    ["ثقله في حقول لا تتغيّر","نحو 70 بالمئة من قوّته التنبّؤية في العقد والمستشفى والقسم، وهي حقول لا يملك الموظّف تغييرها قبل الرفع."],
    ["يستنتج السبب ولا يفحص المستندات","لا يقرأ المرفقات؛ بل يرتّب الأسباب الأرجح من أنماط 19,741 طلباً سابقاً."],
    ["يتدهور بتغيّر سياسات شركات التأمين","يُعاد التدريب كل ربع سنة، وهو إجراء مؤتمت بأمر واحد."],
  ]);
...
/* ============ 14 · MDS / UCAF OPPORTUNITY ============ */
{
  ...
  head(s, "رفع السقف", "الحدّ الأكبر اليوم هو ما لا يراه النموذج",
       "النموذج يعرف «مَن مقدّم الطلب» أكثر ممّا يعرف «ماذا قُدّم فيه». وهذه فجوة قابلة للإغلاق.");
  ...
  const cards = [
    ...
    { x:0.61, n:"3", t:"وما الذي يتغيّر", c:GREEN,
      d:"حقول MDS قابلة للتعديل بالكامل، فينتقل ثقل النموذج من «مَن أنت» إلى «ماذا قدّمت»، ويصير الإنذار قابلاً للتنفيذ." },
  ];
  cards.forEach(o => {
...
/* ============ 15 · ROADMAP + ASK ============ */
{
  ...
  s.addText("أربع مراحل — أوّلها قرار واحد", T({ x:0.6, y:0.82, w:12.1, h:0.62, fontSize:32, bold:true, color:W }));
  const ph = [
    { x:9.89, n:"1", t:"تشغيل تجريبي", d:"مستشفى واحد وأعلى ثلاثة عقود قيمةً، وتُقاس فيه نسبة الإصلاح الفعلية ميدانياً.", k:"4 أسابيع" },
    { x:6.80, n:"2", t:"إلزام الفحص", d:"إدخال سديد خطوةً إلزامية قبل رفع الطلب، وربط مخرجاته بلوحة Power BI القائمة.", k:"4 أسابيع" },
    { x:3.71, n:"3", t:"إضافة MDS", d:"استخراج حقول الاكتمال من UCAF، وإعادة التدريب، وقياس الأثر الثلاثي.", k:"3 أشهر" },
    { x:0.62, n:"4", t:"التعميم والمتابعة", d:"التوسّع على مستوى التجمع، وإعادة تدريب ربع سنوية، ومتابعة أثر الاعتماد شهرياً.", k:"مستمرّ" },
  ];
  ph.forEach(o => {
